...
/** Select the card */
#define SD_Select()  do {MSDC_SPI_CS_Low();} while (0)
/** DeSelect the card */
#define SD_DeSelect() do {MSDC_SPI_CS_High();MSDC_SPI_RecvByte();} while (0)
...
SD_BOOL SD_Init (void)
{
    ...
    SD_DeSelect();
    MSDC_SPI_CLK_Low();
    for (i = 0; i < 10; i++)    MSDC_SPI_SendByte (0xFF);
...
{
    Timer2 = 50;    // 500ms
    MSDC_SPI_RecvByte(); /* Read a byte (Force enable DO output) */
    do {
        if (MSDC_SPI_RecvByte () == 0xFF) return SD_TRUE;
    } while (Timer2);
...
uint8_t SD_SendCommand (uint8_t cmd, uint32_t arg, uint8_t *buf, uint32_t len) 
{
    ...
    MSDC_SPI_SendByte (cmd | 0x40);
    MSDC_SPI_SendByte (arg >> 24);
    MSDC_SPI_SendByte (arg >> 16);
    MSDC_SPI_SendByte (arg >> 8);
    MSDC_SPI_SendByte (arg);
    MSDC_SPI_SendByte (crc_stop); /* Valid or dummy CRC plus stop bit */
...
SD_BOOL SD_ReadConfiguration ()
{
    ...
    switch (CardType)
    {
        ...
        case CARDTYPE_SDV2_HC:
            ...
            for (i=64-16;i;i--) MSDC_SPI_RecvByte();  /* Purge trailing data */            
...
SD_BOOL	SD_WriteBytes(uint32_t sect, uint8_t *buf)
{     
    SD_BOOL res = SD_FALSE;
    
    if ((sect >= SD_SECTOR_END) || (sect <= SD_SECTOR_START))
    {
        return SD_FALSE;
    }
    ...
    {
        SD_SendDataBlock(buf, 0xFE, SECTOR_SIZE);
        res = SD_TRUE;
    }
