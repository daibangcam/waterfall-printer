...
void SD_SendData(void)
{
	...
	{
        c = 0;
        PC_USART_DMA_ReadBytes(pc, 2);
        PC_USART_DMA_WriteBytes(pc, 2);
        if (pc[0] == 'F') c = pc[1];
		c -= 0x30;
		Sdcard_RSec = 0;
		if (c < FILE_NUM)
		...
		if (Sdcard_RSec != 0)
		{
            ...
            PC_USART_DMA_WriteBytes(Sdcard_RBuf, i+1);
			...
			while (Sdcard_RSec < SECTOR_END)
			{
				if ( SD_Read(Sdcard_RSec, Sdcard_RBuf) )
                {
                    ...
                    if ( (Sdcard_RBuf[510] == 'S') && (Sdcard_RBuf[511] == 'D') )
                    {
                        // If this sector is valid, variable i must be RB_READ_SIZE
                        PC_USART_DMA_WriteBytes(Sdcard_RBuf, RB_READ_SIZE);
                    }
